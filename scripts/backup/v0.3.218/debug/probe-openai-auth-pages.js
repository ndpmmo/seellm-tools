...
const userId = `probe_openai_auth_${Date.now()}`;
const sessionKey = `probe_openai_auth_${Date.now()}`;
...
async function dumpState(tabId, label) {
  const state = await evalJson(tabId, `(() => {
    ...
    const fields = Array.from(document.querySelectorAll('input, button, a, [role="button"], form'))
...
async function main() {
  ...
  try {
    ...
    if (PROBE_EMAIL) {
      ...
      await sleep(3000);
      await dumpState(tabId, 'OPENAI_AFTER_EMAIL_SUBMIT');
    }
  } finally {
